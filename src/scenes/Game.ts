...
import {findEmptyCell} from "../helpers";
...
export class Game extends Scene {
    ...
    create() {
        this.snake = new Snake(this, cellWidth * 5 + gridPadding, cellHeight * 2 + gridTopOffset, cellWidth, cellHeight);

        this.snake.snakeObserver.on('isAlive', (isAlive: boolean) => console.log(isAlive))
        ...
        this.physics.add.collider(this.snake.getHead(), this.elements, this.handleFoodCollision, undefined, this);
    ...
    createUI() {
        ...
        this.add.text(160, 20, 'Всього балів', {font: '16px Arial', color: '#ffffff'});
        this.scoreLabel = this.add.text(160, 40, '0', {font: '16px Arial', color: '#ffffff'});
    }
    ...
    addTextInMergedCells() {
        for (let i = 0; i < gridHeight; ++i) {
            ...
            const text = this.add.text(gridPadding, gridTopOffset + cellHeight * i, `${left} - ${right}`, {
    ...
    createCustomGrid() {
        ...
        for (let x = 0; x <= gridWidth; x++) {
            ...
            graphics.lineTo(xPos + gridPadding, gridHeight * cellHeight + gridTopOffset);
        }
    ...
    updateGame() {
        ...
        if (this.ticksCount % 10 === 0) {
            const coordinates = findEmptyCell(gridWidth, gridHeight, cellWidth, cellHeight, this.snake.getSnakePositions(), getPositions(this.elements.getChildren()));
            // spawn food
            if (coordinates) {
    ...
    private handleFoodCollision(_: unknown, object: Phaser.Types.Physics.Arcade.GameObjectWithBody | Tile) {
    ...
    spawnRemovable(point: Phaser.Math.Vector2) {
        const removable = this.physics.add.sprite(point.x - cellWidth / 2, point.y - cellHeight / 2, 'lightning')
            .setOrigin(0, 0)
