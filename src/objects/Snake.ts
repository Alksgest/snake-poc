import {cellHeight, cellWidth, gridHeight, gridPadding, gridTopOffset, gridWidth} from "../settings.ts";
...
export class Snake {
    ...
    constructor(private scene: Phaser.Scene, x: number, y: number, private width: number, private height: number) {
        ...
        scene.physics.add.collider(this.getHead(), this.snakeSegments, this.handleTailCollision, undefined, this);
    }
    ...
    private createHead(x: number, y: number) {
        ...
        const head = this.scene.physics.add.sprite(x - this.width / 2, y - this.height / 2, 'lightning')
    ...
    setDirection(newDirection: Phaser.Math.Vector2) {
        if (this.snakeBody.children.size !== 1 && (newDirection.x === -this.direction.x || newDirection.y === -this.direction.y)) {
