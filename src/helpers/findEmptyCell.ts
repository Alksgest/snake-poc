...
export function findEmptyCell(
  ...
  cellWidth: number,
  cellHeight: number,
  snakePositions: Phaser.Math.Vector2[],
  alreadySpawnedPositions: Phaser.Math.Vector2[],
  ...
  while (!emptyCellFound) {
    ...
    const isOccupied = [...snakePositions, ...alreadySpawnedPositions].some(
      (position) => position.equals(spawnPosition!),
    );
